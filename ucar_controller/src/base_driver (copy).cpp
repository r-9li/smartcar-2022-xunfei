...
double filter::calculate(double input,double coefficient)
{
  if (first_flag)
  {
    last_data=input;
  ...
  if(abs(input-last_data)>coefficient)
  {
    input=last_data;
  }
  last_data=input;
...
void baseBringup::processOdometry(){
  ...
  th_ = yaw;
  x_=x_filter.calculate(x_,0.075);
  y_=y_filter.calculate(y_,0.075);
  th_=th_filter.calculate(th_,10.0);
